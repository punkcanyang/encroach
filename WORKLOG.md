...
### 下一步计划：
- **【核心痛点解决】**：处理玩家反馈的新痛点：**解决小人会扎堆一起做一件事情的问题（AI 聚群拥堵优化）**。需要从空间碰撞或者目标索敌权重上入手，打散或引导小人分头行动。
- **【悬停显示修复】**：由于为解决 Godot 缓存加载失效的问题，我们从 `UIManager.tscn` 删除了旧版的悬停框场景，但尚未在 `UIManager.gd` 中手动通过纯代码 `Node.new()` 的方式实例化并挂载它，造成 `InspectUI` 物理性失踪。明日上线需立刻为其补齐代码实例化挂载流程。
