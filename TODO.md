...
- [x] 【规则集3】建筑环境与引力规则
  - [x] 农田脉冲引力：农田按 Tick 自动积累生长度。成熟度达 100% 时，瞬间爆发极高“觅食引力权重”。
  - [x] 空间距离衰减判定：建筑引力仅在自身半径 R 内有效。距离越近（距中心点），提供的额外权重越大；超出半径的 Agent 感知不到该建筑的引力。
...
- [x] 【规则集7】建筑生命周期与物流机制：
  ...
  - [x] **建筑拆除与返还**：建筑不可移动。拆除建筑时，返还 50% 建造资源（直接入库或原地掉落包裹）。
  - [x] **硬核搬运物流**：除了进食瞬间消耗资源无需回家，矿场等其他资源挖掘后，采集者必须身上携带实体资源，移动步行回“当前有对应存储空位的住所”存放。放入住所的瞬间，该资源才正式计入全局面板可用库存。
...
- [x] 资源显示 (食物、土矿、金属、人口、天数) -> 已在 `StatsPanel` 实装
- [x] Agent 统计面板 -> 已在 `AgentStatsUI` 实装 (快捷键 C)
- [x] 建筑选择面板 -> 已实装为 `BuildingListUI` 清单面板 (快捷键 B)
- [x] 系统日志/事件记录
...
### 人口系统 ✅ DONE
- [x] 人口上限 (由全局所有建筑 `pop_cap` 累加决定)
- [x] 【规则集4】繁殖与人口增长机制：
  - [x] 自然增长：每个住所拥有独立的 `_days_active` 计时器，按各自 `spawn_interval_days` 周期产出新人（1年=10天）。
  - [x] 空间生成逻辑：新生儿在产出该人口的建筑坐标附近随机偏移生成。
  - [x] 前置约束：建筑内部必须有 ≥50 食物储备才触发孵化；全局人口不超过上限。

### 资源循环 🔄 IN PROGRESS
- [x] 食物采集 -> 消耗 -> 死亡的完整循环（已实装 Health 缓冲机制）
- [ ] 资源点自然再生
- [ ] 资源点枯竭与降级（规则集5，待实装）
...
### 调试功能 🔄 IN PROGRESS
- [x] 时间速度控制 (暂停/快进) -> 已在 `StatsPanel` 右上角实装
- [ ] Agent 状态实时查看 -> 目前由 `InspectUI` 基础提供，需专门面板
- [ ] 建筑影响场 Debug 显示
- [ ] 性能监控 (FPS, 实体数量)
...
**核心循环验证：**
1. Agent 自动觅食、消耗、死亡 ✅
2. 建筑放置影响 Agent 行为 ✅（蓝图引力、农田、住所升级链）
3. 资源循环 (采集 -> 搬运 -> 存储 -> 消耗 -> 死亡) ✅
4. 人口生老病死与代际更替 ✅
5. 系统崩溃 = 正常结局 ✅

**后续开发路线（按优先级）：**
1. [x] 【规则集5】野生资源枯竭与降级机制 -> (先前已完成矿物降级与消失)
2. [x] 【规则集7 补完】硬核搬运物流（实体背包步行回库，已加入視覺與減速）
3. [x] 建筑拆除与返还机制 -> 返回 50% 并原地掉落
4. [x] 时间速度控制（暂停/快进）
5. [x] 完善 UI 显示（Agent/建筑清单皆已完成）
6. [x] 系統日誌與事件記錄
7. [ ] 【规则集8】领地辐射与异族战争系统（FUTURE）
