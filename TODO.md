...
- [ ] 【规则集6】实体生老病死：
  - [ ] 生命值系统 (Health 点数)。当饥饿降至 0 时，才开始扣除生命点数。生命值耗尽即因营养不良死亡。
  - [ ] 寿命系统 (随机 50-80 天老死)。
  - [ ] 死亡物质掉落：Agent 死亡时，身上的携带物（如正在搬运的矿石）会掉落在死亡坐标，变为“一次性可拾取资源包”。
- [x] 自动觅食 (饥饿 < 60 时)
- [x] 状态机基础框架
- [ ] 移动系统 (游走、寻路)
  - [ ] **优化点**：解决 AI 扎堆拥堵执行同一目标的问题 (如引入黑板系统或目标预占位机制)
- [ ] 资源采集 (移动到资源点)
- [ ] 行为权重系统 (受建筑影响)
- [ ] 繁殖/生育系统
...
### UI 系统 ⏳ TODO
- [ ] **优先修复**：修复 `InspectUI` 悬停失效的问题（在 `UIManager.gd` 中补充通过纯代码 `Node.new()` 的实例化挂载流程）
- [ ] 资源显示 (食物、人口、天数)
- [ ] Agent 统计面板
- [ ] 建筑选择面板
- [ ] 系统日志/事件记录
